...
export class GeminiExecutor implements ExecutorProvider {
  readonly name = "gemini"
  private apiKey: string
  private model: string
  private projectDir: string

  constructor(apiKey: string, model = "gemini-1.5-flash", projectDir: string = process.cwd()) {
    this.apiKey = apiKey
    this.model = model
  ...
  private buildPrompt(task: Task, prd: string, projectType: string, projectFiles: string): string {
    return `You are a senior software engineer implementing a task for a ${projectType} project.
...
\`\`\``
  ...
  private async call(prompt: string): Promise<ExecutionResult> {
    ...
    if (!response.ok) {
      if (response.status === 429) throw new Error("Gemini: quota exceeded. Wait or switch to gemini-1.5-flash.")
